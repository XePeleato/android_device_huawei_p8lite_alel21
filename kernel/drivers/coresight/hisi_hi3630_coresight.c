		...
		if(csdev->ops) {
			if(csdev->ops->link_ops) {
				if (csdev->ops->link_ops->enable) {
					...
					dev_info(&csdev->dev,"failed csdev->ops->link_ops->enable\n");
				}
			} else {
				dev_info(&csdev->dev,"failed csdev->ops->link_ops\n");
			}
		} else {
			dev_info(&csdev->dev,"failed csdev->ops == NULL\n");
		}
	}
...
err:
	...
	dev_err(&csdev->dev, "coresight: sink switch failed, sources disabled; try again\n");
	return ret;
}
...
int coresight_enable(struct coresight_device *csdev)
{
	...
	if (csdev->type != CORESIGHT_DEV_TYPE_SOURCE) {
		ret = -EINVAL;
		dev_err(&csdev->dev, "coresight: wrong device type\n");
		goto out;
	}
	if (csdev->enable)
		goto out;
	...
	coresight_release_path(&path);
	if (ret)
		dev_err(&csdev->dev, "coresight: enable failed\n");
out:
	up(&coresight_mutex);
	return ret;
}
