 ...
 ******************************************************************************
  ...
  1.日    期   : 2014年1月28日
    作    者   : w56557
    修改内容   : 从《副本Hi6210V100 MODEM寄存器手册_GLB_ON.xml》自动生成
...
/* 寄存器说明：
 bit[31:24]  保留
 bit[23:0]   32K时钟域的自定义中断周期，配置范围是0~512s。单位是32k时钟的周期个数。每隔cpu_int_period个32k时钟合周期，产生1次中断。
...
typedef union
{
    ...
    struct
    {
        unsigned long  cpu_int_period : 24; /* bit[0-23] : 32K时钟域的自定义中断周期，配置范围是0~512s。单位是32k时钟的周期个数。每隔cpu_int_period个32k时钟合周期，产生1次中断。 */
        unsigned long  reserved       : 8;  /* bit[24-31]: 保留 */
    } reg;
