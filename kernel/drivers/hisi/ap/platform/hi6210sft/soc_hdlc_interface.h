 ...
 ******************************************************************************
  ...
  1.日    期   : 2012年8月9日
    作    者   : m53479
    修改内容   : 从《Hi6620V100SOC寄存器手册_HDLC.xml》自动生成
...
/* 寄存器说明：封装子模块转义使能(LCP协议该寄存器无效)。
   详      述：转义使能，从bit 0至 bit 31 分别对应0x00~0x1F 32个字符：
               0：转义禁止；
               1：转义使能。
   UNION 结构：无 */
...
/* 寄存器说明：当frm_in_lii_1dor2d为0，表示待封装输入数据包起始地址寄存器；
   ...
   详      述：当frm_in_lii_1dor2d为0，表示待封装输入数据包起始地址寄存器；
               当frm_in_lii_1dor2d为1，表示封装输入副链表参数起始地址寄存器；
...
/* 寄存器说明：封装输入副链表节点所指待封装输入数据分组起始地址寄存器。
   详      述：封装输入副链表节点所指待封装输入数据分组起始地址寄存器：
...
/* 寄存器说明：对应当前解封装输入链表，同一PPP/IP拨号的以前最近解封装输入链表解出非完整帧的外部存储起始地址，为了支持多个PPP/IP拨号而增加的配置（该地址可能是与原来上报不同的新地址）。
   详      述：对应当前解封装输入链表，同一PPP/IP拨号的以前最近解封装输入链表解出非完整帧的外部存储起始地址，为了支持多个PPP/IP拨号而增加的配置（该地址可能是与原来上报不同的新地址）
   UNION 结构：无 */
