...
typedef union
{
    ...
    struct
    {
        ...
        unsigned short  clkz1    : 1;  /* bit[3]   : SCICLK信号配置模式选择信号。
                                                     0：SCICLK信号配置为下拉（open drain）；
                                                     1：SCICLK信号配置为buffer output。 */
...
typedef union
{
    ...
    struct
    {
        unsigned short  txtide   : 4;  /* bit[0-3] : 设置SCITXTIDEINTR中断的触发值。
                                                     TXFIFO中的字符个数小于或等于SCI_TIDE[txtide]时会触发txtideintr中断。 */
        unsigned short  rxtide   : 4;  /* bit[4-7] : 设置SCIRXTIDEINTR中断的触发值。
...
typedef union
{
    ...
    struct
    {
        unsigned short  txcount  : 4;  /* bit[0-3] : 读该寄存器返回发送FIFO中的字符数，写该寄存器（任意值）将清空发送FIFO。 */
        unsigned short  reserved : 12; /* bit[4-15]: 保留。 */
    } reg;
...
typedef union
{
    ...
    struct
    {
        unsigned short  rxcount  : 4;  /* bit[0-3] : 读该寄存器返回接收FIFO中的字符数，写该寄存器（任意值）将清空接收FIFO。 */
        unsigned short  reserved : 12; /* bit[4-15]: 保留。 */
    } reg;
