...
/* MACRO DEFINITIONS FOR WORKING WITH INDIRECT ACCESS TO SRAM PKA DATA REGISTERS */
	...
	{ \
	   ...
	   DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(HOST_RGF,SRAM_ADDR),((Addr) + LLF_PKI_PKA_DATA_REGS_MEMORY_OFFSET_ADDR)); \
	...
	{ \
	   ...
	   { \
		   DX_HAL_WriteCcRegister(DX_CC_REG_OFFSET(HOST_RGF,SRAM_DATA), ((DxUint32_t*)(ptr))[ii]); \
		   do \
		   { \
		     dummy = DX_HAL_ReadCcRegister(DX_CC_REG_OFFSET(HOST_RGF, SRAM_DATA_READY)); \
		   }while(!(dummy & 0x1)); \
	...
	{ \
	   ...
	   for( ii = 0 ; ii < (SizeWords) ; ii++ ) \
	   { \
		   dummy = (CRYS_COMMON_INVERSE_UINT32_BYTES(((DxUint32_t*)(ptr))[(SizeWords) - ii - 1]));\
