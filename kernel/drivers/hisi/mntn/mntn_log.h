...
#if MDEBUG_LEVEL
#define mlog_d(fmt, ...) pr_info("[D]:%s(%d): " fmt "\n", __FUNCTION__, __LINE__, ##__VA_ARGS__)
#else
#define mlog_d(fmt, ...)
#endif

#if MINFO_LEVEL
#define mlog_i(fmt, ...) pr_info("[I]:%s(%d): " fmt "\n", __FUNCTION__, __LINE__, ##__VA_ARGS__)
#else
#define mlog_i(fmt, ...)
#endif

#if MNOTICE_LEVEL
#define mlog_n(fmt, ...) pr_notice("[N]:%s(%d): " fmt "\n", __FUNCTION__, __LINE__, ##__VA_ARGS__)
#else
#define mlog_n(fmt, ...)
#endif

#if MWARNING_LEVEL
#define mlog_w(fmt, ...) pr_warning("[W]:%s(%d): " fmt "\n", __FUNCTION__, __LINE__, ##__VA_ARGS__)
#else
#define mlog_w(fmt, ...)
#endif

#if MERROR_LEVEL
#define mlog_e(fmt, ...) pr_err("[E]:%s(%d): " fmt "\n", __FUNCTION__, __LINE__, ##__VA_ARGS__)
#else
#define mlog_e(fmt, ...)
#endif
