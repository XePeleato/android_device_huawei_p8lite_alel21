...
VOS_VOID uttest_PCSC_UpdateCardStatus_case1(VOS_VOID)
{
	PS_USIM_STATUS_IND_STRU stMsg;

	g_enAcpuCardStatus = USIMM_CARD_SERVIC_BUTT;
	stMsg.enCardStatus = USIMM_CARD_SERVIC_AVAILABLE;

	PCSC_UpdateCardStatus(&stMsg);
}
...
VOS_VOID uttest_PCSC_AcpuMsgProc_case2(VOS_VOID)
{
	PS_USIM_STATUS_IND_STRU stMsg;

	stMsg.ulMsgName = PS_USIM_GET_STATUS_IND;
