...
static long earpa_ioctl(struct file* file, unsigned int cmd, unsigned long argc)
{
    ...
    switch(cmd) {
        case EAR_PA_OPEN :
            ...
            logd("%s, open earpice pa", __FUNCTION__);
            break;

        case EAR_PA_CLOSE :
            gpio_set_value(g_pdata->gpio_handle, 0);
            logd("%s, close earpice pa", __FUNCTION__);
            break;

        default:
            ret = (long)(-1);
            loge("%s, invalid cmd", __FUNCTION__);
            break;
        }
...
static int earpa_probe(struct platform_device* pdev)
{
    ...
    logd("%s, in function", __FUNCTION__);

    g_pdata = kzalloc(sizeof(struct ear_pa_data), GFP_KERNEL);
    if (NULL == g_pdata) {
        loge("malloc earpice pa memory fail!");
        return -ENOMEM;
    }
    ...
    if (ret) {
        loge("request earpice pa gpio error!");
        goto gpio_error;
    }
    ...
    if (ret) {
        loge("register earpice pa device error!");
    }

    logd("%s, out function", __FUNCTION__);
    return ret;
...
int __init earpa_init(void)
{
    ...
    if (0 != ret) {
        loge("earpa driver register fail , ERROR is %d\n", ret);
    }
