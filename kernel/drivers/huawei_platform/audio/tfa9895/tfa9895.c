...
static int tfa9895_i2c_read(char *rxData, int length)
{
    ...
    if (0 > ret) {
        pr_err("%s: transfer error %d", __FUNCTION__, ret);
        return ret;
    }
         ...
         size_t nbytes, loff_t *pos)
{
    ...
    if (0 > ret) {
        pr_err("%s: i2c read error %d", __FUNCTION__, ret);
	    kfree(kern_buf);
        return ret;
    }
		...
		const char __user *buf, size_t nbytes, loff_t *ppos)
{
    ...
    if (0 > ret) {
        pr_err("%s: i2c write error %d", __FUNCTION__, ret);
        return ret;
    }
...
static int tfa9895_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
    ...
    this_client = client;
	pr_info("%s: enter, device '%s' , addr %x\n", __func__, id->name, this_client->addr);

    if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
        pr_err("%s: i2c check functionality error", __FUNCTION__);
        ret = -ENODEV;
		return ret;
    }
    ...
    if (ret){
        pr_err("%s: tfa9895_device register failed (%d)", __FUNCTION__, ret);
    }
