                 ...
                 char *buf)
{
    ...
    list_for_each_entry_safe(tx, ttmp, &asc_tx_handle_list, node){
        ...
        len += snprintf(s, PAGE_SIZE, "Tx %s: ref=%d, ap_wake_cp(%d)=%d, cp_ready(%d)=%d, polar=%d, auto_delay=%d mS\n",
        ...
        list_for_each_entry_safe(user, tuser, &tx->user_list, node) {
            infor = &user->infor;
            len += snprintf(s + len, PAGE_SIZE - len, "       user %s: ref=%d\n", infor->name, atomic_read(&user->count));
        }
    }
    ...
    list_for_each_entry_safe(rx, rtmp, &asc_rx_handle_list, node){
        ...
        len += snprintf(s + len, PAGE_SIZE - len, "Rx %s: ref=%d, cp_wake_ap(%d)=%d, ap_ready(%d)=%d, polar=%d\n", \
                 ...
                 char *buf)
{
    ...
    if(rx){
        return snprintf(s, ASC_NAME_LEN, "done\n");
    }else{
        return snprintf(s, ASC_NAME_LEN, "null\n");
    }
}
