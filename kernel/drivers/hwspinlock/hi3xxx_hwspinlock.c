...
static int hisi_hwspinlock_probe(struct platform_device *pdev)
{
	...
	match = of_match_device(hisi_hwlock_of_match, dev);
	if (!match)
		return -EINVAL;
	...
	for (i = 0, hwlock = &bank->lock[0]; i < num_lock; i++, hwlock++, hwspinlock_info++) {
		...
		hwspinlock_info->address = io_base + addr[i / locks_per_register];
...
static int hisi_hwspinlock_remove(struct platform_device *pdev)
{
	...
	struct hisi_hwspinlock *lock_temp = (struct hisi_hwspinlock *)(bank->lock[0].priv);
	void __iomem *io_base = lock_temp->address - 0x400;
	int ret;
...
static struct platform_driver hisi_hwspinlock_driver = {
	...
	.driver		= {
		.name	= MODULE_NAME,
		.owner	= THIS_MODULE,
		.of_match_table	= hisi_hwlock_of_match,
	},
};
