...
void memlist_dump(void)
{
	...
	list_for_each_entry(node, &idlenode_list, node_head){
		ALOGD("DUMP_IDLE:phy=0x%8lx w=%6d h=%6d stride=%6d  size=%6d ref=%d this=0x%8x father=0x%8x\n",
			...
			node->cpubuf.size,
			node->ref,
			(unsigned int)node->thisnode,
			(unsigned int)node->father
			);
	}

	list_for_each_entry(node, &allocatednode_list, node_head){
		ALOGD("DUMP_BUSY:phy=0x%8lx w=%6d h=%6d stride=%6d  size=%6d ref=%d this=0x%8x father=0x%8x\n",
			node->cpubuf.phy_addr,
			node->cpubuf.bytew,
			node->cpubuf.byteh,
			node->cpubuf.stride,
...
static mem_node_t *node_alloc_direct(struct gen_pool *pool, int size, int stride, int byteh)
{
	...
	ALOGD_INFO("MEMORY info:direct alloc node=0x%8x size=%d\n", (unsigned int)newnode, size);
...
static void node_free_direct(struct gen_pool *pool, mem_node_t * tofreenode)
{
	...
	if(father != NULL && father->ref == 0){
		pr_info("MEMORY info:direct free node=0x%8x size=%d\n", (unsigned int)father, father->cpubuf.size);
...
static void node_free(struct gen_pool *pool, mem_node_t * tofreenode)
{
	...
	do{
		...
		if(twins == NULL){
			...
			freenode = father;
			ALOGD_INFO("cpubuffer info:merge this_ref=%d father_ref=%d father=0x%8x\n", ref, father->ref, (unsigned int)father);
		}
